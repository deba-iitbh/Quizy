...
// var url = "mongodb+srv://deba:123@cluster0.ydvmzz9.mongodb.net/?retryWrites=true&w=majority";
...
io.on('connection', (socket) => {
    ...
    socket.on('time', function (data) {
        var time = data.time / 30;
        time = time * 100;
        var playerid = data.player;
        var player = players.getPlayer(playerid);
        player.gameData.score += Math.max(0, time);
    });
        ...
        MongoClient.connect(url, {
            ...
            dbo.collection("kahootGames").find(query).toArray(function (err, res) {
                ...
                if (res[0].questions.length >= game.gameData.question) {
                    ...
                    const csvContent = playersInGame.map(player => `${player.name}, ${player.gameData.score}`).join('\n');
                    ...
                    fs.writeFile(csvFileName, csvContent, 'utf8', (err) => {
                        if (err) {
                            ...
                            console.log(`CSV file "${csvFileName}" has been created successfully.`);
                        }
                    });
    ...
    socket.on('requestDbNames', function (passcode) {
